...
def GaussNoise(image_path, path_test, image, fold, kernel=[3,7,9,15]):
    for k in kernel:
        try:
            img = cv2.imread(image_path)
            dst = cv2.GaussianBlur(img,(k,k),cv2.BORDER_DEFAULT)
            cv2.imwrite(path_test+"-GaussNoise-"+str(k)+"/"+fold+"/"+image[:-4]+".png", dst)
        except:
            print("**Errore nell'elaborazione della foto - Gaussian Noice ")

def JPEGCompr(image_path, path_test, image, fold, qfac = ['1','10','20','30','40', '50', '60', '70', '80', '90']):
    for q in qfac:
        try:
            im = Image.open(image_path)
            im.save(path_test+"-jpegQF"+str(q)+"/"+fold+"/"+image[:-4]+'.jpg',format='jpeg', subsampling=0, quality=int(q))
        except:
...
for _, row in guidance_csv.iterrows():
